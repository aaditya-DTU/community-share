...
export const getMyProfile = async (req, res, next) => {
  try {
    ...
    const [user, myItems, borrowedItems, incomingRequests, reviews] =
      await Promise.all([
        ...
        Item.find({ owner: userId })
          .select("title category isAvailable condition createdAt images")
          .sort({ createdAt: -1 })
          .lean(),
    ...
    res.json({ user, myItems, borrowedItems, incomingRequests, reviews });
  } catch (err) {
    next(err);
  }
};
...
export const getUserById = async (req, res, next) => {
  try {
    const user = await User.findById(req.params.id)
      .select("name trustScore averageRating reviewCount totalBorrows totalLends avatar createdAt")
      .lean();
